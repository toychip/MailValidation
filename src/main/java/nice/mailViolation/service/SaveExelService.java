...
//        }
        if (reason == ReasonIneligibility.C) {
            return "기안자: 일반 사원 | 팀장한테 결재받았지만 보직좌(실장 or 본부장)를 참조하지 않음";
        }
        if (reason == ReasonIneligibility.D) {
            return "기안자: 일반 사원 | 팀장, 실장, 본부장 중 아무에게도 결재를 받지 않음";
        }
        if (reason == ReasonIneligibility.E) {
            return "기안자: 팀장 | 실장 or 본부장 중 아무에게도 결재를 받지 않음";
        }
        if (reason == ReasonIneligibility.F) {
            return "기안자: 실장 | 본부장에게 결재를 받지 않음";
        }
        if (reason == ReasonIneligibility.G) {
            return "기안자: 일반사원 | 경영지원실 팀장, 실장, DB관리자 중 한 명에게 결재를 받았지만, 본인 부서의 보직좌를 참조하지 않음";
        }
